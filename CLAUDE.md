...
各レッスンのタイトル直後（「このレッスンでわかること」の直前）に吹き出しを置く。

```html
<section class="flex items-start gap-3">
  <div class="shrink-0 w-10 h-10 bg-amber-400 rounded-full flex items-center justify-center text-lg shadow">🕵️</div>
...
L10（最終章）のみ強調バージョン（`border-2 border-amber-300` + タイトル付き）を使う。
...
コータのおじさんが麻績村在住という設定で、「なぜ村が縮んでいるのか」をデータで調べるのが全体の動機。
...
| L | セリフの焦点 |
|---|---|
| L1 | 麻績村を初めて紹介し、データを見ようと誘う |
| L2 | 大きな市と小さな村の人口差を実感させる |
| L3 | 「村から人がいなくなった」を折れ線グラフで確かめる |
| L4 | 麻績村が77市町村のランキングで何番目か調べる |
| L5 | 麻績村は8年で445人減ったと数字で実感させる |
| L6 | %で比べることで−16%の深刻さを伝える |
| L7 | 男女どちらが多く減っているか調べる |
| L8 | 長野県全体でも同じ傾向かを確認する |
| L9 | このまま続くと2033年は何人になるか予測する |
| L10 | 最終局面として自分なりの答えを出すよう促す |
